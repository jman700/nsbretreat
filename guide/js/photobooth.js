...
(function () {
  ...
  (function () {
    ...
    style.textContent = [
      // Countdown overlay — positioned relative to #pb-camera (position:relative in CSS)
      '.pb-countdown {',
      '  display: none;',
      '  position: absolute;',
      '  inset: 0;',
      ...
      '  display: flex;',
      '}',
      // Timer/flip icon button active state
      '.pb-icon-btn.active {',
      '  background: var(--accent, #b8967e) !important;',
      '  border-color: var(--accent, #b8967e) !important;',
      '  color: #fff !important;',
      '}'
    ].join('\n');
  ...
  // ── Output dimensions (single-shot) ───────────────────
  var OUT_W = 900, OUT_H = 1200;

  // ── Strip output dimensions ────────────────────────────
  var STRIP_W      = 900;
  var STRIP_PHOTO  = 840;  // square photo size (840×840)
  var STRIP_MARGIN = 30;   // (900-840)/2 horizontal margin each side
  var STRIP_GAP    = 14;   // gap between photos
  var STRIP_VPAD   = 16;   // top padding
  var STRIP_BRAND  = 108;  // brand footer height
  // Total height: 16 + 840 + 14 + 840 + 14 + 840 + 16 + 108 = 2688
  ...
  // Strip state
  var stripShots     = [];   // captured canvases (up to 3)
  var stripShotIdx   = 0;    // which slot is currently active (0, 1, 2)

  // ── Supabase (silent background upload) ───────────────
  ...
  var singleVf  = document.getElementById('pb-single-vf');
  var stripVf   = document.getElementById('pb-strip-vf');
  ...
  function enterStripMode() {
    if (singleVf) singleVf.style.display = 'none';
    if (stripVf)  stripVf.style.display  = '';
    stripShots   = [];
    stripShotIdx = 0;
    resetStripSlots();
    moveVideoToSlot(0);
  ...
  function exitStripMode() {
    // Return video to single viewfinder if it was moved
    if (video && singleVf && video.parentNode !== singleVf) {
      singleVf.insertBefore(video, singleVf.firstChild);
    }
    if (singleVf) singleVf.style.display = '';
    if (stripVf)  stripVf.style.display  = 'none';
    stripShots   = [];
    stripShotIdx = 0;
  }
  ...
  function resetStripSlots() {
    for (var i = 0; i < 3; i++) {
      var slot = document.getElementById('pb-sslot-' + i);
      if (!slot) continue;
  ...
  function captureStripShot() {
    ...
    var vw = video.videoWidth  || STRIP_PHOTO;
    var vh = video.videoHeight || STRIP_PHOTO;
    ...
    if (facing === 'user') {
      ctx.save();
      ctx.translate(capSize, 0);
      ctx.scale(-1, 1);
      ctx.drawImage(video, sx, sy, size, size, 0, 0, capSize, capSize);
      ctx.restore();
    } else {
      ctx.drawImage(video, sx, sy, size, size, 0, 0, capSize, capSize);
    }
    ...
    if (currentSlot) currentSlot.appendChild(preview);

    if (stripShotIdx < 3) {
      // Advance to next slot
      moveVideoToSlot(stripShotIdx);
      if (btn) btn.disabled = false;
    } else {
      // All 3 shots taken — compose the final strip
      setTimeout(function () { buildFinalStrip(); }, 280);
    }
  }
  ...
  function buildFinalStrip() {
    ...
    canvas.toBlob(function (blob) {
      ...
      // Return video to single viewfinder before showing preview
      if (video && singleVf && video.parentNode !== singleVf) {
        singleVf.insertBefore(video, singleVf.firstChild);
      }
      showPreview();
  ...
  document.getElementById('pb-flip-btn').addEventListener('click', function () {
    ...
    if (activeFrame === 'strip') {
      // Reset strip shots on camera flip
      stripShots   = [];
      stripShotIdx = 0;
    }
    startCamera();
  ...
  document.getElementById('pb-capture-btn').addEventListener('click', function () {
    ...
    var doCapture = activeFrame === 'strip' ? captureStripShot : capture;
  ...
  // Stop camera on page hide / tab switch
